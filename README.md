...
``` addToCar ```// Добавляет переданный товар в корзину в корзину.  
``` removeFromCar ```// Убирает переданный товар из корзины.  
``` clearCar ```// Очищает корзину.  
...
### ClientAPI  
Назначение: Отвечает за дополнение исходного класса Api.   
...
```total``` // Меняет итоговую сумму на переданное значение.  
### Card
...
Методы:  
```payment``` // Меняет классы кнопок метода оплаты.  
```address``` // Меняет содержимое поля addredd в форме.  
### ContactsForm
